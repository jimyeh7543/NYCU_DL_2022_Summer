...
class Td3:
    ...
    def update(self, t):
        ...
        self._update_behavior_network(t, self.gamma)
    ...
    def _update_behavior_network(self, t, gamma):
        ...
        ## TODO ##
        with torch.no_grad():
            noise = torch.from_numpy(self._action_noise.sample()).to(torch.float32).to(self.device)
            a_next = self._target_actor_net(next_state) + noise
            a_next = a_next.clamp(-self.max_action, self.max_action)
        ...
        critic_loss_1 = criterion(q_value_1, q_target)
        ...
        critic_loss_2 = criterion(q_value_2, q_target)
        ...
        if t % self.policy_delay == 0:
            ...
            actor_loss = -self._critic_net_1(state, action).mean()
...
def main():
    ...
    parser.add_argument('-d', '--device', default='cuda:4')
    ...
    parser.add_argument('--seed', default=20220827, type=int)
