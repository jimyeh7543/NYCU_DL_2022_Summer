...
class evaluation_model():
    def __init__(self, device, n_gpu):
        # modify the path to your own path
        checkpoint = torch.load('logs/classifier/checkpoint.pth', map_location=device)
        ...
        if n_gpu > 1:
            self.resnet18 = nn.DataParallel(self.resnet18, list(range(n_gpu)))
        self.resnet18.eval()
        self.classnum = 24
